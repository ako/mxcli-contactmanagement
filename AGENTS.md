# Mendix Project: mxcli-contactmanagement
...
```bash
./mxcli -p ContactManagement.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p ContactManagement.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check ContactManagement.mpr   # Validate project
./mxcli docker check -p ContactManagement.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p ContactManagement.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p ContactManagement.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p ContactManagement.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p ContactManagement.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p ContactManagement.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'ContactManagement.mpr';
```
...
./mxcli check script.mdl -p ContactManagement.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p ContactManagement.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p ContactManagement.mpr

# With colored output
./mxcli lint -p ContactManagement.mpr --color

# List available rules
./mxcli lint -p ContactManagement.mpr --list-rules

# Output as SARIF
./mxcli lint -p ContactManagement.mpr --format sarif > results.sarif
```
...
./mxcli report -p ContactManagement.mpr

# JSON report
./mxcli report -p ContactManagement.mpr --format json

# HTML report
./mxcli report -p ContactManagement.mpr --format html
```
